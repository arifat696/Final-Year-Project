...
Original file is located at
    https://colab.research.google.com/drive/19Tu16O4OV37GVywllJNL1vLe2xa1jmRK
"""
...
hungarian.columns = header

hungarian_cleaned = hungarian.drop(['slope', 'ca', 'thal'], axis=1)
print(hungarian_cleaned)

# Remove rows with '?' in any column (already replaced with NaN in the cleaning step)
hungarian_cleaned_no_missing = hungarian_cleaned.replace('?', pd.NA).dropna()

# Convert all columns in the dataset to numeric, coercing errors
hungarian_numeric = hungarian_cleaned_no_missing.apply(pd.to_numeric, errors='coerce')
...
cleaned_data_hungarian = remove_outliers(hungarian_numeric, continuous_columns)

# Comparing the original data with the cleaned data
cleaned_shape_hungarian = cleaned_data_hungarian.shape

print(hungarian_numeric.shape, cleaned_shape_hungarian)

!pip install ctgan
!pip install table_evaluator

categorical_features = ['sex', 'cp', 'fbs', 'restecg', 'exang', 'num']

from ctgan import CTGAN
ctgan = CTGAN(verbose=True)
ctgan.fit(cleaned_data_hungarian, categorical_features, epochs = 2000)
...
print(cleaned_data_hungarian.shape, cleaned_data1.shape)
table_evaluator = TableEvaluator(cleaned_data_hungarian, cleaned_data1, cat_cols= categorical_features)
