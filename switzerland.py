...
Original file is located at
    https://colab.research.google.com/drive/1EI1Ht_A3N3usZ9hnYvHgV6evQFYU-FJN
"""
...
switzerland.columns = header

switzerland_cleaned = switzerland.drop(['chol','fbs','ca', 'thal'], axis=1)
print(switzerland_cleaned)

# Remove rows with '?' in any column (already replaced with NaN in the cleaning step)
switzerland_cleaned_no_missing = switzerland_cleaned.replace('?', pd.NA).dropna()

# Convert all columns in the dataset to numeric, coercing errors
switzerland_numeric = switzerland_cleaned_no_missing.apply(pd.to_numeric, errors='coerce')
...
cleaned_data_switzerland = remove_outliers(switzerland_numeric, continuous_columns)

# Comparing the original data with the cleaned data
cleaned_shape_switzerland = cleaned_data_switzerland.shape

print(switzerland_numeric.shape, cleaned_shape_switzerland)

!pip install ctgan
!pip install table_evaluator

categorical_features = ['sex', 'cp', 'restecg', 'exang', 'slope', 'num']

from ctgan import CTGAN
ctgan = CTGAN(verbose=True)
ctgan.fit(cleaned_data_switzerland, categorical_features, epochs = 2000)
...
print(cleaned_data_switzerland.shape, cleaned_data1.shape)
table_evaluator = TableEvaluator(cleaned_data_switzerland, cleaned_data1, cat_cols= categorical_features)
